...
new class extends Component {
    ...
    public function with(): array
    {
        ...
        if ($timesheets->count() > 0 && ! $this->selectedTimesheetId) {
            $this->selectedTimesheetId = 'tab-'.$timesheets->first()->id;
        }
            ...
            <div class="flex flex-col lg:flex-row gap-0 bg-base-100 rounded-xl shadow-sm border border-base-200 overflow-hidden min-h-[600px]">
                ...
                <div class="w-full lg:w-80 border-b lg:border-b-0 lg:border-r border-base-200 bg-base-200/20 flex flex-col pt-2 overflow-y-auto max-h-[600px] scrollbar-hide">
                    @foreach($timesheets as $item)
                        <button
                            wire:click="$set('selectedTimesheetId', 'tab-{{ $item->id }}')"
                            class="px-8 py-6 text-left transition-all border-l-4 {{ $selectedTimesheetId == 'tab-'.$item->id ? 'bg-base-100 border-primary shadow-sm' : 'border-transparent hover:bg-base-100/30 opacity-60 hover:opacity-100' }}"
                        >
                            <span class="block text-xs font-black uppercase tracking-widest {{ $selectedTimesheetId == 'tab-'.$item->id ? 'text-primary' : 'text-base-content/60' }} truncate mb-1">
                                {{ $item->title }}
                            </span>
                ...
                <div class="flex-1 p-8 lg:p-16 bg-base-100 overflow-y-auto max-h-[600px]">
                    @php
                        $selectedItem = $timesheets->firstWhere('id', (int) str_replace('tab-', '', (string) $selectedTimesheetId));
                    @endphp
